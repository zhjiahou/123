...
def data_split(path, n_predictions, n_next):  # 定义函数
    ...
    data = pd.DataFrame(data, columns=['Wind_Speed', 'Wind_Direction', 'Temp_Air', 'Humidity', 'Pressure',
                                       'power'])
    ...
    for i in range(data.shape[0] - n_predictions - n_next + 2):  # 第一维的空间行长度-一组的长度-间隔长度""""""
        ...
        b = data.iloc[i:(i + n_predictions), -1:]  # 一个冒号对应一个[]
        Y.append(b)
...
for i in range(q, len(training_set_scaled)-a):
    x_train.append(training_set_scaled[i-q:i, 0])
    y_train.append(training_set_scaled[i:i+a, 0])
...
x_train = np.reshape(x_train, (x_train.shape[0], q,1))

for i in range(q, len(test_set)-a):
    x_test.append(test_set[i - q:i, 0])
    y_test.append(test_set[i:i+a, 0])
print("y_test:",y_test)

# 测试集变array并reshape为符合RNN输入要求：[送入样本数， 循环核时间展开步数， 每个时间步输入特征个数]
x_test, y_test = np.array(x_test), np.array(y_test)
x_test = np.reshape(x_test, (x_test.shape[0], q, 1))
...
real_stock_price = sc.inverse_transform(test_set[q+a:])
# 画出真实数据和预测数据的对比曲线
